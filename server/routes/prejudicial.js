    ...
    try {
        ...
        let query = `
            SELECT 
                ...
                dr."ФИО/Наименование" as client_fio,
                dr."№ л/с" as account_number
            FROM 
                activity_logs al
            LEFT JOIN 
                users u ON al.user_id = u.id
            LEFT JOIN 
                dos_rabota dr ON al.record_id = dr.id
            WHERE 
                al.table_name = 'dos_rabota'
        `;
        ...
        console.log('📡 Запрос истории prejudicial');
        ...
        console.log(`✅ История prejudicial: ${formattedRows.length} записей`);
        res.json(formattedRows);
        
    } catch (error) {
        console.error('❌ Ошибка в /api/prejudicial/history:', error);
        res.status(500).json({ 
            success: false,
